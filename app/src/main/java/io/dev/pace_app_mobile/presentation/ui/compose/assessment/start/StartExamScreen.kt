...
fun StartExamScreen(
    ...
    /**
     * Call assessment API only once on first screen load.
     */
    LaunchedEffect(Unit) {
        ...
        when (userType) {
            UserType.GUEST -> {
                ...
                val selectedEmail = loginResponse?.studentResponse?.email ?: ""
    ...
    /**
     * Handle navigation separately so API call isn’t repeated.
     */
    ...
    /**
     * Handle assessment status separately (reactive to studentResponse changes).
     */
    LaunchedEffect(studentResponse?.assessmentStatus) {
        when (studentResponse?.assessmentStatus) {
            "DONE" -> assessmentViewModel.onDoneClick()
            else -> Timber.d("Assessment not yet done.")
        }
    }
    ...
    Scaffold(
        ...
        Column(
            ...
            // --- Begin Button ---
            CustomDynamicButton(
                onClick = { assessmentViewModel.onBeginClick() },
                content = "Let's Begin",
